...
>
> **修复 WorkBuddy 5.6+ 用户推送失效的问题。** 5.6 起客户端把 `settings.json` 里的
> `botToken` / `channelId` 改为加密信封（`{"$wbEncrypted":1,"envelope":"<base64>"}`），
> 密钥由客户端原生层经 IPC 下发，**外部脚本无法解密**。旧版会把凭据对象拼成
> `Bearer [object Object]`，服务端返回具有误导性的 `errcode=-14 session timeout`，
> **消息发不出去**。本版改从客户端轮询游标取材，**自动适配，无需手工干预**。
>
> 同时新增 **`--hook-status` 入口自检**——排查「收不到消息」时先确认 hook 是否真的执行：
